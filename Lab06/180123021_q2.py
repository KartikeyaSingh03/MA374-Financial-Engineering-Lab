...
def plotGraph(company, returns, frequency, nbins, market):
    ...
    plt.hist(returns, bins = nbins, edgecolor = 'black', density = 1, label = 'Returns')
    plt.plot(X,Y,color = 'red', label = 'N(0, 1)')
    plt.title('%s Returns for %s (%s)'%(frequency,company,market))
    plt.xlabel('Returns')
    plt.ylabel('Frequency')
    plt.legend(loc = 'best')
    plt.savefig(frequency + "_" + "normal_" + market + "_" + company)
    plt.clf()
