...
def avgStockPrice(S0,M,u,d):
    ...
    avg_prices = [0]*len(prices[-1])

    for i in reversed(range(M+1)):
        for j in range(len(prices[-1])):
            avg_prices[j] += (prices[i][int(j/(2**(M-i)))])/(M+1)
...
def calcOptionPrice(setNo,M,T,S0,r,K,sig):
    ...
    avg_prices = avgStockPrice(S0,M,u,d)	
   
    Call = [max(0,s-K) for s in avg_prices]
    Put = [max(0,K-s) for s in avg_prices]

    while(len(Call) > 1):
        C = []
        P = []
        for i in range(0,len(Call),2):
            C.append((p*Call[i] + (1-p)*Call[i+1]) * np.exp(-r*dt))
            P.append((p*Put[i] + (1-p)*Put[i+1]) * np.exp(-r*dt))		
...
def analyzeSensitivity2(S0,K,T,M,r,sig,param1,param2):
    ...
    if param1 == "S0" and param2 == "K":
        ...
        for Sigma in range(1750,2250,10):
            ...
            Put2.append(put2_temp)
            Call2.append(call2_temp)
    figname = "Q2_%s_%s.png"%(param1,param2)
